	...
	<form method="post">
		<div class="box-body">
			...
			<div class="form-group">
				...
				<input name="password" id="password" type="password" class="form-control" value="<?php echo $this->sanitize($this->model->password); ?>" autocomplete="new-password">
			</div>
			<div class="form-group">
				...
				<select name="timezone" id="timezone" class="form-control">
					...
					$selected_timezone = (!empty($this->model->timezone)) ? $this->model->timezone : Config::$value['default_timezone'];

					foreach ($timezones as $timezone) {
						echo '<option value="'.$timezone.'"'.(($selected_timezone == $timezone) ? ' selected' : '').'>'.$timezone.'</option>';
					}
					?>
				</select>
			</div>
